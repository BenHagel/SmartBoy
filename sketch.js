...
var p5_neuronDefaultDiam = 20;
var p5_networkOffsetX = 0;
var p5_networkOffsetY = 4;
var p5_networkOffsetZ = 4;
...
function setup(){
	//createCanvas(windowWidth, AwindowHeight);
	...
	OVERLORD_RAND = new CustomRandom("snxeakfdddsdfy seed", CHelper__B.hasher_256);

	boy = new SmartBoy({
		"type": "smartboy",		//Smartboy
		"layers": [4, 49, 4],	//The policy network
		"gene": [0.5, 0.5, 0.5, 0.5, 0.5, 0.2],	//values for all the god parameters required
		//Physical distancing constants
		"unit_distance": p5_neuronApartDefaultOffset,
		"unit_diam": p5_neuronDefaultDiam,
		"unit_offset_x": p5_networkOffsetX,
		"unit_offset_y": p5_networkOffsetY,
		"unit_offset_z": p5_networkOffsetZ
	});

	console.log("created new boy");
...
function drawPastScores(){
	for(let i = 0;i < Gen.trendField.length;i+=3){
		...
		text("Generation: " + i + " : " + Gen.trendField[i].f, 30, i * 30 + 50);
	}
}
...
function drawNeurons(){
		...
		if(animateTheVisualNetworkModel){
			translate(0, Math.sin(0.01*frameCount) * boy.layers[2][boy.layers[2].length-1].p5_y/3 - boy.layers[2][boy.layers[2].length-1].p5_y/2, 0);
	    ...
	    for(let k = 0;k < boy.totalNeurons();k++){
			...
			//else noStroke();
			push();
			translate(n.p5_x, n.p5_y, n.p5_z)
			sphere(n.p5_diam);
			pop();
...
function drawNexts(){
	...
	for(let i = 0;i < boy.oracle.nexts.length;i++){
	    ...
	    noStroke();
	    ellipse(boy.oracle.nexts[i].f.p5_x + (vec.x * (frameCount%24) / 24),
